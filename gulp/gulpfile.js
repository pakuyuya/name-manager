...
(function(Exec){
    ...
    Exec.clean_webcontent = function(cb){
        del(
            [
                SRC_DIR + 'css/**/*',
                SRC_DIR + 'js/**/*',
    ...
    Exec.copy_lib = function(){
        gulp.src([SRC_DIR + 'lib/**/*'], {base : SRC_DIR})
            .pipe(gulp.dest(BUILD_DIR));
    };

    Exec.ts_compile = function(){
...
// ビルドディレクトリ初期化
gulp.task('initdir', Exec.init_dir);

gulp.task('clean-php', ['initdir'], Exec.clean_php);
gulp.task('clean-lib', ['initdir'], Exec.clean_lib);
gulp.task('clean-webcontent', ['initdir'], Exec.clean_webcontent);

// 前回ビルド結果削除
gulp.task('clean', ['clean-php', 'clean-lib', 'clean-webcontent']);

// PHPファイルコピー
gulp.task('copy-php',['clean-php'], Exec.copy_php);
gulp.task('copy-php-noclean', Exec.copy_php);

// HTMLファイルコピー
gulp.task('copy-html', Exec.copy_html);

// ライブラリコピー
gulp.task('copy-lib',['clean-lib'], Exec.copy_lib);
gulp.task('copy-lib-noclean', Exec.copy_lib);

// Typescriptコンパイル
gulp.task('typescript-compile', Exec.ts_compile);

// Compassコピー
gulp.task('compass-compile', Exec.compass_compile);
